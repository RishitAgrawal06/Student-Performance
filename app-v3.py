...
def plotly_barplot(students_df, column_var = 'average score'):
    ...
    return fig
def plt_testprep_bars(students_df):
    students_df['score sum'] = students_df['math score'] + students_df['reading score'] + students_df['writing score']
    students_df['score average'] = (students_df['math score'] + students_df['reading score'] + students_df['writing score'])/3
    fig = px.histogram(students_df, x="test preparation course", y="score average",color='gender', barmode='group', height=400, histfunc='avg', category_orders={"gender": ["male", "female"]})
    fig.update_layout(xaxis_title_text='Test Preparation Course', yaxis_title_text='Score Average')
    return fig
...
st.plotly_chart(plt_testprep_bars(students_df))

col1, col2 = st.columns(2)    
with col1:
    score_selection = st.radio("Select Certain Scores", ('Writing score', 'Reading score', 'Math score', 'Score average')).lower()
    
with col2:
    st.write("Parental Level of Education & Test Scores Average")
    st.plotly_chart(plotly_barplot(students_df, score_selection))
st.subheader("Rigors of students backgrounds are tested through the means of exams... Through this dataset we have compared the exam scores of men and women based on variables that we found that would affect academic performence.")
